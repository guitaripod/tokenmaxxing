...
fn paint_header(cr: &Context) {
    ...
    text(cr, "AMPERE", MARGIN + 108.0, MARGIN + 58.0, 52.0, FontWeight::Bold, theme::CYAN, false);
...
fn paint_footer(cr: &Context, height: f64) {
    ...
    let _ = cr.stroke();
    text(cr, "github.com/guitaripod/quota", MARGIN, y, 21.0, FontWeight::Normal, theme::MUTED, true);
    text_right(cr, "ampere 0.1.0", W - MARGIN, y, 21.0, FontWeight::Normal, theme::MUTED, true);
}
...
pub fn default_output() -> std::path::PathBuf {
    ...
    dir.join(format!("ampere-{stamp}.png"))
}
