...
struct TokenmaxxingTray {
    ui_tx: async_channel::Sender<TrayEvent>,
    to_worker: Sender<FromUi>,
    icon: Vec<Icon>,
}

impl Tray for TokenmaxxingTray {
    fn id(&self) -> String {
        "dev.guitaripod.tokenmaxxing".into()
    }
    ...
    fn menu(&self) -> Vec<MenuItem<Self>> {
        vec![
            StandardItem {
                label: "Show / hide tokenmaxxing".into(),
...
pub fn start(ui_tx: async_channel::Sender<TrayEvent>, to_worker: Sender<FromUi>) -> bool {
    ...
    match tray.spawn() {
        ...
        Err(error) => {
            eprintln!("tokenmaxxing: system tray unavailable ({error}); window stays open");
            false
        }
    }
}
...
/// Render the tokenmaxxing mark and convert Cairo's premultiplied native-endian
