...
![Prism](../assets/prism-sharecard.png)
...
```sh
make run        # swift build → assemble Prism.app → ad-hoc codesign → open
make install    # same, into /Applications
```

Prism is a dock-less menu-bar agent (`LSUIElement`). Click the glass-cube icon to open the popover; the ☰ / slider menu has the interface-scale picker, share-card export, launch-at-login, open console, and quit.
...
```sh
Prism --export [path.png]     # renders the quota state to a PNG and exits
```
